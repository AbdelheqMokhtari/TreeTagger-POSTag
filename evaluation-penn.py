...
# Define file paths
file_paths = {
    "NN" : os.path.join(data_folder, "NNC_test_text.txt"),
    ...
    "WDT" : os.path.join(data_folder, "that_pronoun.txt")
}
...
def get_that_tag(sentence):
    ...
    for tag in tags:
        ...
        if len(parts) >= 2 and parts[0].lower() == "that":
            return parts[1].split("/")[0]  # Extract main POS tag
    return None  # "that" not found

# List of possible labels
true_labels = list(file_paths.keys())  # ['NN', 'RB', 'IN', 'DT', 'WDT']
predicted_labels = []
...
for true_label, file_path in file_paths.items():
    if os.path.exists(file_path):
        ...
        output_file_path = os.path.join(output_folder, f"results_{true_label}.txt")
